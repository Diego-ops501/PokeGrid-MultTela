...
(async () => {
  ...
  ok(b.includes("stCache[i] = { t: Date.now(), d }; dtPush();") && b.includes("JSON.stringify(dtMeus()) !== dtMeusKey"), 'leitura nova de um painel refaz "Meu Ditto" se o time mudou (antes o select nascia vazio e ficava)');
  ...
  ok(s.includes('<button id="dittoBtn"') && s.indexOf('<button id="dittoBtn"') > s.indexOf('<button id="tierBtn"') && s.indexOf('<button id="dittoBtn"') < s.indexOf('<button id="count"'), 'botao Ditto no menu, logo abaixo da tierlist');
